...
mean_std_caregivers <- data %>%
  filter(CRGVPERS == 1 | CRGVPERS == 2) %>% #filters out non-caregivers and those that did not preform identified tasks
...
Q3 <- Q3 <- round(as.data.frame(mean_std_caregivers,2)) #getting 2 decimal places where needed as outlined by format, and from tibble to dataframe
...
Menhlth_marital <- data %>%
  ...
  filter(MENTHLTH <= 30 & MARITAL <= 6) #pulling out the refused to answer Martial responses and those that answered NONE(88), Not Sure(77) and Refused(99) treating them as outliers
...
#Part 2 - variables chosen are - 

#Problem 10
final_data <- data %>%
  select(VETERAN3, MENTHLTH, MARITAL, EXRACT11) %>%
  na.omit(EXRACT11) %>% #Removing none or NA responses
  filter(MENTHLTH != 88 | MARITAL != 9) #Removing None and Refused Responses
Q10 <- final_data
...
ggplot(final_data_no_out) +
  geom_histogram(mapping = aes(x=VETERAN3), binwidth = 0.5)
# Visualizations for MENTHLTH
ggplot(final_data_no_out) +
  geom_boxplot(mapping = aes(MENTHLTH))
ggplot(final_data_no_out) +
  geom_histogram(mapping = aes(x=MENTHLTH), binwidth = 0.5)
# Visualizations for MARITAL
ggplot(final_data_no_out) +
  geom_boxplot(mapping = aes(MARITAL))
ggplot(final_data_no_out) +
  geom_histogram(mapping = aes(x=MARITAL), binwidth = 0.5)
# Visualizations for EXRACT11
ggplot(final_data_no_out) +
  geom_boxplot(mapping = aes(EXRACT11))
ggplot(final_data_no_out) +
  geom_histogram(mapping = aes(x=EXRACT11), binwidth = 0.5)
